      ...
      <div className="grid gap-6 md:grid-cols-2">
        {aboutStats.map((item, index) => {
          ...
          return (
            ...
            >
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm bg-white text-[#D4A72C]">
        ...
        <div className="space-y-10">
          {selectedDay.sessions.map((session) => (
            <div key={`${session.time}-${session.title}`} className="grid gap-4 md:grid-cols-[160px_1fr]">
              <div className="text-sm font-bold uppercase tracking-wider text-[#D4A72C]">
                {session.time}
              </div>
              <div>
                    ...
                    {session.speakers
                      ...
                      .map((speaker) => (
                        <p key={speaker.name} className="text-sm text-gray-600">
                          <span className="font-semibold text-[#D4A72C]">{speaker.name}</span>
        ...
        {whyAttend.map((item, index) => {
          ...
          return (
            <div key={item.title} className="flex gap-4 border-b border-gray-100 pb-6 last:border-0">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-sm bg-[#FCF8F3] text-[#D4A72C]">
                <Icon className="h-6 w-6" />
              </div>
              <div>
    ...
    <section>
      ...
      <div className="max-w-xl rounded-sm border border-gray-100 bg-[#FCF8F3] p-6 md:p-8">
        ...
        <div className="mt-4 space-y-1 text-sm font-semibold text-[#031A34]">
          ...
          <a href="tel:+919148319993" className="block hover:text-[#D4A72C]">
            +91 91483 19993
          </a>
        </div>
      </div>
