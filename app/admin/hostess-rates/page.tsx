...
import { Plus, Edit, Trash2, Save, X, Sparkles, Users, CheckCircle, XCircle } from "lucide-react";
import {
...
export default function AdminHostessRatesPage() {
    ...
    const calculateLocalStatistics = (categoriesList: HostessCategory[]) => {
        const total = categoriesList.length;
        const active = categoriesList.filter(cat => cat.isActive).length;
        const avgRate = categoriesList.length > 0
            ? categoriesList.reduce((sum, cat) => sum + cat.ratePerDay, 0) / categoriesList.length
            : 0;
        setStatistics({ total, active, avgRate });
    };
    ...
    const fetchCategories = async () => {
        try {
            ...
            if (response.success) {
                ...
                calculateLocalStatistics(fetchedCategories);
            } else {
                toast.error(response.message || "Failed to load hostess categories");
            }
        } catch (error) {
    ...
    const handleToggleStatus = async (id: string, currentStatus: boolean) => {
        try {
            ...
            if (response.success) {
                ...
                calculateLocalStatistics(updatedCategories);
            } else {
        ...
        <div className="space-y-6 p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <button
                    onClick={() => {
                        ...
                        setFormData({ category: 'A', ratePerDay: '', workingHours: '8', description: '' });
                        setShowModal(true);
                    }}
            ...
            {loading ? (
                ...
                <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-12 text-center">
                    ...
                    <button onClick={() => setShowModal(true)} className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-lg hover:from-orange-600 hover:to-red-700 transition">
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex gap-1">
                                        <button
                                            ...
                                            className={`p-1.5 rounded-lg transition ${cat.isActive ? 'text-gray-400 hover:text-red-400 hover:bg-red-500/20' : 'text-gray-400 hover:text-green-400 hover:bg-green-500/20'}`}
                                        ...
                                        <button onClick={() => handleDelete(cat.id, cat.category)} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-500/20 rounded-lg transition">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-[#0E2044]/80 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 rounded-2xl max-w-md w-full p-6 border border-gray-700">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-white">{editingCategory ? 'Edit Hostess Category' : 'Add New Hostess Category'}</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value as 'A' | 'B' })} className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-orange-500" disabled={!!editingCategory}>
                                    <option value="A">Category A (Premium)</option>
                                    <option value="B">Category B (Standard)</option>
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-2">Rate per Day (₹) <span className="text-red-400">*</span></label>
                                <div className="relative">
                                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">₹</span>
                                    <input type="number" value={formData.ratePerDay} onChange={(e) => setFormData({ ...formData, ratePerDay: e.target.value })} className="w-full pl-8 pr-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-orange-500" placeholder="5000" required />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-2">Working Hours per Day <span className="text-red-400">*</span></label>
                                <input type="number" value={formData.workingHours} onChange={(e) => setFormData({ ...formData, workingHours: e.target.value })} className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-orange-500" placeholder="8" required />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-300 mb-2">Description <span className="text-red-400">*</span></label>
                                <textarea rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:border-orange-500" placeholder="Describe the hostess services included" required />
                            </div>
                        </div>
                        <div className="flex gap-3 mt-6 pt-4 border-t border-gray-700">
                            <button onClick={() => setShowModal(false)} className="flex-1 px-4 py-2 border border-gray-600 rounded-lg text-gray-300 hover:bg-gray-700 transition">Cancel</button>
                            <button onClick={async () => {
                                ...
                                try {
                                    if (editingCategory) {
                                        const response = await updateHostessCategory(editingCategory.id, { ratePerDay: parseInt(formData.ratePerDay), workingHours: parseInt(formData.workingHours), description: formData.description });
                                        if (response.success) {
                                        ...
                                        const response = await createHostessCategory({ category: formData.category, ratePerDay: parseInt(formData.ratePerDay), workingHours: parseInt(formData.workingHours), description: formData.description });
                                        if (response.success) {
                                ...
                                <Save size={16} /> {editingCategory ? 'Update Category' : 'Create Category'}
                            </button>
                        </div>
                    </div>
                </div>
