    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-white overflow-hidden">
      <Container>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6 mb-8 sm:mb-10 lg:mb-12 flex-wrap">
          ...
          <Button
            href="/articles/"
            className="border border-[#D4A72C] text-[#D4A72C] bg-[#D4A72C] hover:bg-[#ff8c00] hover:text-white px-8 py-3 text-xs sm:text-sm font-bold uppercase tracking-wider transition-all duration-300 rounded-sm w-full sm:w-auto text-center"
          >
            VIEW ALL ARTICLES
          </Button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {featuredArticle && (
            <div className="lg:w-[65%] group cursor-pointer min-w-0">
              ...
              <div className="mt-4">
                <p className="text-[#D4A72C] text-xs font-semibold font-sans">
                  {formatDate(featuredArticle.publishedDate)}
                </p>
                <Link href={`/articles/${featuredArticle.slug}`}>
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-[#031A34] mt-2 leading-snug hover:text-[#D4A72C] transition">
          ...
          <div className="lg:w-[35%] flex flex-col gap-6 min-w-0">
            {restArticles.map((article) => (
              <div key={article.slug} className="grid grid-cols-[35%_65%] sm:grid-cols-[40%_60%] gap-3 sm:gap-4 items-start group cursor-pointer">
                ...
                <div>
                  <p className="text-[#D4A72C] text-[10px] sm:text-xs font-semibold font-sans">
                    {formatDate(article.publishedDate)}
                  </p>
                  <Link href={`/articles/${article.slug}`}>
                    <h4 className="text-xs sm:text-sm md:text-base font-bold text-[#031A34] mt-1 leading-snug hover:text-[#D4A72C] transition line-clamp-2">
                      {article.title}
                    </h4>
                  </Link>
